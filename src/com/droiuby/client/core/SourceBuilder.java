...
public class SourceBuilder {
	...
	public static JDefinedClass buildModel(JCodeModel cm, String targetClass,
			Class<?> baseClass, boolean includeOverridable) {
		try {
			...
			if (!baseClass.isInterface() && !hasDefaultConstructor(baseClass)) {
				// check if there is a default constructor
				defineConstructor(baseClass, constructor, executionBundleObject);
			}
	...
	private static void defineConstructor(Class<?> baseClass,
			JMethod constructor, JVar executionBundleObject) {
		for (Constructor<?> c : baseClass.getConstructors()) {
	...
	public static void build(String targetClass, Class<?> baseClass,
		...
		try {
			...
			buildModel(cm, targetClass, baseClass, true);
			cm.build(f);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
